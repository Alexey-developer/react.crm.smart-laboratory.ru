...
export const EditOperatorProfilePage: React.FC = () => {
  const { entityId } = useParams()

  const [translated_phrase] = useTranslation('global')
  SetPageTitle(
    `${translated_phrase(
      'MenuItems.Telephony.operator_profiles'
  ...
  React.useEffect(() => {
    if (isLoading || isFetching || !operatorProfile) {
      return
    }
    ...
    formItems.forEach(value => {
      state.initialValues[value.name] = operatorProfile[value.name]
    })
  ...
  }, [isLoading, isFetching, operatorProfile])
