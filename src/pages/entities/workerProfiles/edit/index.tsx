...
export const EditWorkerProfilePage: React.FC = () => {
  const { entityId } = useParams()

  const [translated_phrase] = useTranslation('global')
  SetPageTitle(
    `${translated_phrase(
      'MenuItems.Employees.profiles'
  ...
  React.useEffect(() => {
    if (isLoading || isFetching || !workerProfile) {
      return
    }
    ...
    formItems.forEach(value => {
      state.initialValues[value.name] = workerProfile[value.name]
    })
  ...
  }, [isLoading, isFetching, workerProfile])
