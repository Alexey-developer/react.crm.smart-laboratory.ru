...
export const EditCompanyDialNumberPage: React.FC = () => {
  const { entityId } = useParams()

  const [translated_phrase] = useTranslation('global')
  SetPageTitle(
    `${translated_phrase(
      'MenuItems.Telephony.company_dial_numbers'
  ...
  React.useEffect(() => {
    if (isLoading || isFetching || !companyDialNumber) {
      return
    }
    ...
    formItems.forEach(value => {
      state.initialValues[value.name] = companyDialNumber[value.name]
    })
  ...
  }, [isLoading, isFetching, companyDialNumber])
