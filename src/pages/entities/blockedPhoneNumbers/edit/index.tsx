...
export const EditBlockedPhoneNumberPage: React.FC = () => {
  const { entityId } = useParams()

  const [translated_phrase] = useTranslation('global')
  SetPageTitle(
    `${translated_phrase(
      'MenuItems.Telephony.blocked_phone_numbers'
  ...
  React.useEffect(() => {
    if (isLoading || isFetching || !blockedPhoneNumber) {
      return
    }
    ...
    formItems.forEach(value => {
      state.initialValues[value.name] = blockedPhoneNumber[value.name]
    })
  ...
  }, [isLoading, isFetching, blockedPhoneNumber])
