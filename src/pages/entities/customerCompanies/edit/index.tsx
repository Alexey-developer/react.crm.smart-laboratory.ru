...
export const EditCustomerCompanyPage: React.FC = () => {
  const { entityId } = useParams()

  const [translated_phrase] = useTranslation('global')
  SetPageTitle(
    `${translated_phrase(
      'MenuItems.WorkingWithCustomers.companies'
  ...
  React.useEffect(() => {
    if (isLoading || isFetching || !customerCompany) {
      return
    }
    ...
    formItems.forEach(value => {
      state.initialValues[value.name] = customerCompany[value.name]
    })
  ...
  }, [isLoading, isFetching, customerCompany])
