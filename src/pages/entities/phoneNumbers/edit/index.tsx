...
export const EditPhoneNumberPage: React.FC = () => {
  const { entityId } = useParams()

  const [translated_phrase] = useTranslation('global')
  SetPageTitle(
    `${translated_phrase(
      'MenuItems.Telephony.phone_numbers'
  ...
  React.useEffect(() => {
    if (isLoading || isFetching || !phoneNumber) {
      return
    }
    ...
    formItems.forEach(value => {
      state.initialValues[value.name] = phoneNumber[value.name]
    })
  ...
  }, [isLoading, isFetching, phoneNumber])
